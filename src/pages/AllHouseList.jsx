...
export function AllHouseList() {
  ...
  const getLastPage = (response) => {
    const linkHeader = [...response.headers].find((header) => {
        return header[0] === 'link';
    })[1];

    const lastUrl = linkHeader.split(',').find((item) => {
        return item.includes('last');
    }).trim().split(';')[0].replace('<','').replace('>','');
